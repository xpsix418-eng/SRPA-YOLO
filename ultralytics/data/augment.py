...
class SmallObjectCopyPasteBoxes:
    """Copy small box crops to free image regions without requiring segmentation masks."""

    def __init__(
        self, p: float = 0.0, max_size: float = 32.0, max_pastes: int = 3, max_ioa: float = 0.1, trials: int = 20
    ) -> None:
        """Configure probability, small-object size, and collision limits."""
        self.p = p
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        """Paste unchanged rectangular crops and append their bounding-box labels."""
        instances = labels["instances"]
        if self.p == 0 or random.random() > self.p or len(instances) == 0 or len(instances.segments):
            return labels
        ...
        candidates = np.flatnonzero(
            (sizes.min(axis=1) >= 2.0) & (sizes.max(axis=1) <= self.max_size)
        ).tolist()
        if not candidates:
            return labels
        ...
        classes = labels["cls"]
        added_boxes, added_classes = [], []
        for index in candidates[: self.max_pastes]:
            x1, y1, x2, y2 = boxes[index]
            ix1, iy1 = max(0, int(math.floor(x1))), max(0, int(math.floor(y1)))
            ix2, iy2 = min(w, int(math.ceil(x2))), min(h, int(math.ceil(y2)))
            patch = im[iy1:iy2, ix1:ix2].copy()
            ph, pw = patch.shape[:2]
            if ph < 2 or pw < 2 or ph > h or pw > w:
                continue

            occupied = np.concatenate((boxes, np.asarray(added_boxes, dtype=np.float32).reshape(-1, 4)), axis=0)
            for _ in range(self.trials):
                nx1 = random.randint(0, w - pw)
                ny1 = random.randint(0, h - ph)
                candidate = np.array([nx1, ny1, nx1 + pw, ny1 + ph], dtype=np.float32)
                inter_w = np.maximum(
                    0.0, np.minimum(candidate[2], occupied[:, 2]) - np.maximum(candidate[0], occupied[:, 0])
                )
                inter_h = np.maximum(
                    0.0, np.minimum(candidate[3], occupied[:, 3]) - np.maximum(candidate[1], occupied[:, 1])
                )
                ioa = inter_w * inter_h / max(float(pw * ph), 1.0)
                if np.all(ioa < self.max_ioa):
                    im[ny1 : ny1 + ph, nx1 : nx1 + pw] = patch
                    added_boxes.append(candidate)
                    added_classes.append(classes[[index]])
                    break

        if added_boxes:
            instances.update(np.concatenate((boxes, np.asarray(added_boxes, dtype=np.float32)), axis=0))
            labels["cls"] = np.concatenate((classes, *added_classes), axis=0)
            labels["img"] = im
        return labels
...
def v8_transforms(dataset, imgsz: int, hyp: IterableSimpleNamespace, stretch: bool = False):
    ...
    small_copy_paste = SmallObjectCopyPasteBoxes(
        p=getattr(hyp, "small_object_copy_paste", 0.0),
        max_size=getattr(hyp, "small_object_max_px", 32.0),
        max_pastes=getattr(hyp, "small_object_max_pastes", 3),
    )
    pre_transform = Compose([mosaic, affine, small_copy_paste])
