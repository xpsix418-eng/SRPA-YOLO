# Ultralytics 🚀 AGPL-3.0 License - https://ultralytics.com/licenseSimAMA
...
from ultralytics.nn.Attentions import CBAM, FastCBAM
from ultralytics.nn.Attentions import SimAM
    ...
    def fuse(self, verbose=True):
        ...
        if not self.is_fused():
            ...
            for index, layer in enumerate(layers[:-1]):
                if isinstance(layer, FoldableNormalizedConcat):
                    layer.fuse_into(layers[index + 1])
            for m in self.model.modules():
                ...
                if isinstance(
                    m, (AuxDirectDetect, RepSharedLeadAuxDetect, RepSharedLeadPyramidAuxDetect, RepSharedP5AuxDetect)
                ):
                    m.fuse()  # remove the training-only auxiliary head
                if isinstance(m, RepSharedChannelScaleDetect):
                    m.fuse()  # fold static classification channel gains into cv3
                if isinstance(m, RepSharedP2TaskResidualDetect):
                    m.fuse()  # merge coarse box/class projections while keeping finest task residuals
                if isinstance(m, RepSharedP2DualTaskResidualDetect):
                    m.fuse()  # merge coarse projections while keeping dual finest residuals
                if isinstance(m, RepSharedMergedDetect):
                    m.fuse()  # merge box and class projections into one exact 1x1
                if isinstance(m, RepSharedSplitTaskDetect):
                    m.fuse()  # merge split-task box and class projections into one exact 1x1
                if isinstance(
                    m,
                    (
                        RepSharedPrivateResidualDetect,
                    ...
                    m.fuse()  # fold the frozen base and private class residual into one deployment path
...
class DetectionModel(BaseModel):
    ...
    def init_criterion(self):
        ...
        auxiliary_mode = getattr(self.model[-1], "auxiliary_train", False)
        if auxiliary_mode == "lead_guided":
            return LeadGuidedAuxDetectLoss(self)
        if auxiliary_mode:
            return AuxiliaryDetectLoss(self)
...
def parse_model(d, ch, verbose=True):
    ...
    # Args
    legacy = bool(d.get("legacy", True))  # allow YAMLs to opt into the efficient modern Detect classification head
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            if m in {C3k2, C3k2NAM}:  # for M/L/X sizes
                legacy = False
                if scale in "mlx":
                    args[3] = True
            if m is A2C2f:
            ...
            if m is C2fCIB:
                legacy = False
        elif m in {AIFI, AdaptiveSparseAIFI}:
            args = [ch[f], *args]
        ...
        elif m in {CBAM, FastCBAM}:
            c2 = ch[f]
            args = [c2,*args]
        elif m in {CoordAtt, ECA, ECASpatial, EMA, FastEMA, FrequencyGuidedAttention, WaveletFeatureRefinement}:
            c2 = ch[f]
            args = [c2, *args]
        elif m is SimAM:
             # SimAM不需要通道参数，但YOLO解析器会传递，所以我们需要特殊处理
            c1 = ch[f]
            args = []  # 清空args，或者保留必要的参数
            m_ = nn.Sequential(*(m(*args) for _ in range(n))) if n > 1 else m(*args)
        elif m in {Concat_BiFPN, FastNormalizedConcat}:
            c2 = sum(ch[x] for x in f)
        elif m is FoldableNormalizedConcat:
            channels = [ch[x] for x in f]
            args.append(channels)
            c2 = sum(channels)
        elif m is P5SemanticInject:
            channels = [ch[x] for x in f]
            args = [channels, *args]
            c2 = channels[0]
        elif m in {SPDConv}:
